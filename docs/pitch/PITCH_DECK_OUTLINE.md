...
| 7 | **Fraud & compliance** | Shell-company flag (CR < 12 months) caught mid-pipeline · full SAMA audit log per scoring event · **auditor-ready XAI**: exact factor attribution + fair-lending adverse-action + input-level fairness attestation · **forward-looking 6-mo default probability** (transparent, hybrid signals) · **autonomous underwriter agent** drafts + answers, on zero-PII aggregates · human officer on every loan. | `13_officer_dashboard_full.png` + `screenshots/6_ai_privacy_proof.png` |
