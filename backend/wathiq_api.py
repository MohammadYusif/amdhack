"""
Real Wathiq (developer.wathq.sa) Commercial Registration API client.
...
Two consequences for how this is used:
  1. verify_cr() in wathiq_simulation.py must NOT let this masked dummy
  ...
  2. fetch_live_proof() exposes the *raw* masked response on demand, openly
     labeled as a real, live Wathiq call — proof the integration genuinely
...
logger = logging.getLogger("wathiq_api")
...
def _credentials() -> tuple[str | None, str | None]:
    return os.environ.get("WATHIQ_CONSUMER_KEY"), os.environ.get("WATHIQ_BASE_URL")
...
def _map_response(cr_number: str, raw: dict) -> dict | None:
    ...
    if not name or not status_name:
        logger.warning("WATHIQ_LIVE: unrecognized response shape for CR %s — falling back. Raw keys: %s",
                        cr_number, list(raw.keys()))
        return None
    if _is_masked_sandbox_dummy(name):
        logger.info("WATHIQ_LIVE: CR %s returned masked sandbox dummy data — falling back to simulation "
    ...
    return {
        ...
        "message_en": "CR verified live via Wathiq" if not risk_flag else "Warning: requires review",
        "source": "WATHIQ_LIVE",
    }


def fetch_real_cr(cr_number: str) -> dict | None:
    """Attempt a real Wathiq lookup. Returns None on any failure (including
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.info("WATHIQ_LIVE: CR %s not in sandbox dataset — falling back to simulation.", cr_number)
        elif resp.status_code == 403:
            logger.info("WATHIQ_LIVE: 403 for CR %s — app not entitled for this resource. Falling back.", cr_number)
        else:
            logger.warning("WATHIQ_LIVE: unexpected status %s for CR %s — falling back.", resp.status_code, cr_number)
    except requests.RequestException as e:
        logger.warning("WATHIQ_LIVE: request failed for CR %s (%s) — falling back.", cr_number, e)
...
def fetch_live_proof(cr_number: str = LIVE_PROOF_SAMPLE_CR) -> dict:
    """
    Make a real, uncached, unfiltered call to Wathiq and return the raw
    ...
    if not key or not base_url:
        return {
            "live": False,
            "error": "WATHIQ_CONSUMER_KEY not configured",
        }
    ...
    return {
        ...
        "note_en": (
            "This is a real, live call to the official Wathiq API, made just now. "
            "The Trial sandbox masks part of the company name with 'x' characters for privacy — "
            "that's Wathiq's own behavior, not a Mihan simulation. The paid production tier "
